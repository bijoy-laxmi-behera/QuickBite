...
const createMenuItem = async (req, res) => {
  try {
    ...
    // Category optional — cloud kitchen items don't need categories
    let categoryExists = null;
    if (category) {
      categoryExists = await Category.findOne({ _id: category, vendor: req.user._id });
      if (!categoryExists) {
    ...
    const { mealSlot, dayOfWeek } = req.body;

    const menuData = {
      name:        name.trim(),
      description: description || '',
      price:       Number(price) || 0,
      mealSlot:    mealSlot   || "lunch",   // lunch | dinner | both
      dayOfWeek:   dayOfWeek  || "daily",   // daily | Monday | Tuesday ...
      vendor:      req.user._id,
    ...
    // Only set category if a valid value was provided
...
const getProfile = async (req, res) => {
  try {
    ...
    // Get restaurant document to read type (Cloud Kitchen vs Restaurant)
    const restaurant = await Restaurant.findOne({ owner: req.user._id }).select(
      "name type cuisine address location image logo description deliveryTime minOrder isOpen isApproved rating specialOffers menuPlan"
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        restaurantInfo: {
          ...
          finalStatus,
          nextOpen,
          type:           restaurant?.type || "Restaurant",  // ← KEY for cloud kitchen detection
