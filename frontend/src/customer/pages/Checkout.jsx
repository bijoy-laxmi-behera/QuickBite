...
// ─── Your Razorpay TEST key (public key — safe in frontend) ──────────────────
...
function Checkout({ cart, setCart, setPage }) {
  ...
  // ── Fetch addresses + user + cart coupon ──────────────────────────────────
  useEffect(() => {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const openRazorpay = (rzpOrder, backendOrderId) => {
    return new Promise((resolve, reject) => {
      const options = {
        ...
        image:        "/logo.png",               // optional — put your logo in /public
        ...
        // Which methods to show — based on user's selection
        method: payment === "upi"        ? { upi: true,  card: false, netbanking: false, wallet: false }
               ...
               : undefined, // cod — not sent to Razorpay
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      // 1. Save new address if not using a saved one
      ...
      // 2. Get customer GPS location (best-effort)
      ...
      if (!customerLocation && navigator.geolocation) {
        ...
        } catch { /* location denied — that's fine */ }
      }
      ...
      // Step B: Create a Razorpay order on YOUR backend
      //         Your backend calls Razorpay API and returns { id, amount, currency }
      const rzpCreateRes = await API.post("/customer/payments/create-razorpay-order", {
        amount:       total,        // in ₹ — backend converts to paise (×100)
        currency:     "INR",
        receipt:      `receipt_${Date.now()}`,
        paymentMethod: payment,
      });

      if (!rzpCreateRes.data.success) {
        alert(rzpCreateRes.data.message || "Could not initiate payment");
        return;
      }

      const rzpOrder = rzpCreateRes.data.data; // { id, amount, currency }

      // Step C: Open Razorpay checkout modal — wait for user to pay
      let paymentResult;
      try {
        paymentResult = await openRazorpay(rzpOrder, rzpOrder.id);
      } catch (err) {
      ...
      // Step D: Verify payment on YOUR backend + place order atomically
      //         Backend verifies Razorpay signature, then creates the order
      ...
      if (verifyRes.data.success) {
        ...
        alert(verifyRes.data.message || "Payment verification failed. Contact support.");
      }
  ...
  const renderAddressSection = () => {
    ...
    return (
      <>
              ...
              {addresses.map((addr) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    {addr.location && <p className="text-xs text-gray-400 mt-0.5">📍 Live location saved</p>}
                  </div>
                </label>
              ))}
            </div>
        ...
        <button
          type="button"
          onClick={() => { setShowLocationPicker(!showLocationPicker); if (addresses.length > 0) setSelectedAddressId(""); }}
      ...
      <div className="max-w-2xl mx-auto px-4 pt-4">
            ...
            <div className="mt-3 bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-500 mt-0.5">
                  Clicking "Pay &amp; Place Order" will open the Razorpay checkout window.
                  Your payment details are encrypted with 256-bit SSL and never stored on our servers.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-5">
          ...
          <div className="border-t border-dashed border-gray-100 mt-3 pt-3 space-y-1.5 text-sm">
            ...
            {discountAmt > 0 && (
              <div className="flex justify-between text-green-600"><span>Coupon Discount</span><span>− ₹{discountAmt}</span></div>
            )}
