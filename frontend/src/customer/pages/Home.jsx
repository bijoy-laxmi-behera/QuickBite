...
import {
  FaSearch, FaHeart, FaUser, FaBell, FaShoppingCart, FaSpinner,
} from "react-icons/fa";
...
function Home({ setCart, setSelectedRestaurant ,setFavourites,setPage}) {
  ...
  const filters = ["Popular", "Fast Delivery", "Tiffin", "Healthy", "Subscription", "Nearest"];
...
useEffect(() => {
      ...
      if (response.data.success && response.data.data.length > 0) {
        ...
        const transformedDishes = response.data.data
          ...
          .map((item) => ({
            ...
            discount: item.discount || `${Math.floor(Math.random() * 20) + 5}% OFF`,
            image: item.item?.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&q=80",
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await API.get(`/customer/restaurants/search?q=${encodeURIComponent(searchQuery)}`);
      if (response.data.success) {
    ...
    <div className="bg-gray-100 min-h-screen pb-8">
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-5 text-xl shrink-0 self-end sm:self-auto">
          <div className="relative cursor-pointer" onClick={() => navigate("/customer/notifications")}>
          ...
          <div className="relative cursor-pointer" onClick={() => navigate("/customer/cart")}>
              ...
              {popularDishes.map((dish) => (
                ...
                >
                  ...
                  <div className="p-2.5 sm:p-3">
                    <h3 className="text-xs sm:text-sm font-semibold text-gray-800 truncate">{dish.name}</h3>
                    <div className="flex items-center justify-between mt-1.5">
                      <p className="text-orange-500 font-bold text-sm sm:text-base">{dish.price}</p>
                      <button 
                    ...
                    {dish.ordersCount && (
                      <p className="text-[10px] text-gray-400 mt-1">⭐ {dish.ordersCount}+ orders</p>
                    )}
                  </div>
                </div>
              ))}
