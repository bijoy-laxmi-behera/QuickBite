...
function OrderSuccess({ setPage, orderId: propOrderId }) {
  ...
  // ✅ Only use fresh orderId from props or navigation state — never stale localStorage
  const freshId = propOrderId || location.state?.orderId;
  if (freshId) localStorage.setItem("lastOrderId", freshId); // update if fresh
  const orderId = freshId || null;
