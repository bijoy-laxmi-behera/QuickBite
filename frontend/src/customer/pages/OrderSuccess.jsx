...
function OrderSuccess({ setPage, orderId: propOrderId }) {
  ...
  // ✅ FIXED: resolve orderId from props → location state → localStorage
  // Orders.jsx sets localStorage.setItem("lastOrderId", order.id) before navigating here
  const orderId =
    propOrderId ||
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      ...
      try {
        ...
        // Fallback: use cached order from localStorage
        try {
          const saved = JSON.parse(localStorage.getItem("lastOrder") || "null");
          if (saved) {
            setOrder(saved);
            setStatus(saved.deliveryStatus || saved.status || "confirmed");
          }
        } catch {}
      } finally {
        setLoading(false);
  ...
  // ✅ NEW: graceful fallback if no orderId found at all
  if (!orderId && !order) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 px-6 text-center">
        <span className="text-5xl">🛍️</span>
        <h2 className="text-xl font-extrabold text-gray-700">No order selected</h2>
        <p className="text-gray-400 text-sm">Go back to your orders and tap one to view details.</p>
        <button
          onClick={() => setPage ? setPage("orders") : navigate("/customer/orders")}
          className="mt-2 bg-orange-500 text-white px-6 py-3 rounded-2xl font-bold hover:bg-orange-600 transition"
        >
          Back to Orders
        </button>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-sm mx-auto px-4 py-8 flex flex-col items-center text-center">
        ...
        <div className={`transition-all duration-700 ${show ? "scale-100 opacity-100" : "scale-50 opacity-0"} mb-5`}>
          <div className={`w-28 h-28 rounded-full flex items-center justify-center mx-auto shadow-xl ring-8 ${isCancelled ? "bg-red-100 ring-red-50" : "bg-green-100 ring-green-50"}`}>
            <div className={`w-20 h-20 rounded-full flex items-center justify-center shadow-lg ${isCancelled ? "bg-gradient-to-br from-red-400 to-red-500" : "bg-gradient-to-br from-green-400 to-emerald-500"}`}>
              <span className="text-4xl">{isCancelled ? "❌" : isDelivered ? "🎉" : "✓"}</span>
            </div>
          </div>
        </div>
        ...
        <div className={`transition-all duration-700 delay-150 ${show ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"} w-full`}>
          ...
          {isCancelled && <div className="mb-6" />}
          ...
          {/* OTP card — only for active orders */}
          ...
          {/* Progress steps — hidden for cancelled orders */}
          {!isCancelled && (
          ...
          <button
            onClick={() => setPage ? setPage("orders") : navigate("/customer/orders")}
            className="w-full text-gray-500 py-3.5 rounded-2xl font-semibold border border-gray-200 hover:bg-gray-50 transition text-sm mb-2"
          >
            Back to Orders
          </button>
          ...
          <button
            ...
            className="w-full text-orange-500 py-3 rounded-2xl font-semibold hover:bg-orange-50 transition text-sm"
          >
            Back to Home
          </button>
        </div>
