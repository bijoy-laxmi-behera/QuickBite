      ...
      <div className="max-w-2xl mx-auto px-4 -mt-8 space-y-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black text-gray-800">Kitchen Menu</h3>
              <p className="text-xs text-gray-400 mt-0.5">Items available from your kitchen</p>
            </div>
            {menu.length > 0 && (
          ...
          {/* Category filter */}
          {menu.length > 0 && (() => {
            const cats = ["All", ...new Set(menu.map(i => (typeof i.category === "object" ? i.category?.name : i.category) || "Other").filter(Boolean))];
            const filtered = activeCategory === "All" ? menu : menu.filter(i => {
              const c = typeof i.category === "object" ? i.category?.name : i.category;
              return c === activeCategory;
            });
            return (
              <>
                {/* Category pills */}
                <div className="flex gap-2 overflow-x-auto px-5 py-3 border-b border-gray-50 scrollbar-hide">
                  {cats.map(cat => (
                ...
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {filtered.map((item) => {
                    const isVeg = item.isVeg || item.type === "veg";
                    ...
                    return (
                      <div key={item._id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {catName && (
                            <span className="text-[10px] bg-white border border-gray-200 text-gray-400 px-2 py-0.5 rounded-full mt-1 inline-block">
                              {catName}
                            </span>
                          )}
                          {item.description && (
                          ...
                          {!item.isAvailable && (
                            <span className="text-[10px] text-red-500 font-semibold mt-1 inline-block">Unavailable</span>
                          )}
                        </div>
                      </div>
                    );
          ...
          {!menuLoading && menu.length === 0 && (
            <div className="text-center py-10">
              ...
              <p className="text-gray-400 text-sm font-medium mt-2">No menu items yet</p>
              <p className="text-gray-300 text-xs mt-1">The kitchen hasn't added items yet</p>
            </div>
          )}
        </div>
