...
function ItemModal({
  ...
  const handleSubmit = async () => {
    ...
    // Price required only for restaurant; cloud kitchen uses subscription pricing
    if (!isCloudKitchen && !form.price) {
    ...
    try {
      ...
      // For cloud kitchen: set price=0, isAvailable=true, skip category
      const payload = { ...form };
      if (isCloudKitchen) {
        payload.price = 0;
      ...
      if (item?._id)
        await API.put(`/vendor/menu/${item._id}`, fd, {
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    value={form.mealSlot}
                    onChange={(e) => f("mealSlot", e.target.value)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => f("dayOfWeek", e.target.value)}
                    className="mt-1 w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-orange-400 transition bg-white"
                  >
              ...
              <div
                className={`text-xs font-medium px-3 py-2 rounded-xl ${
                  form.mealSlot === "lunch"
                    ...
                    : form.mealSlot === "dinner"
                      ? "bg-blue-50 text-blue-600"
                      : "bg-purple-50 text-purple-600"
                }`}
              >
                This item will appear in subscribers'{" "}
                {form.mealSlot === "both" ? "Lunch & Dinner" : form.mealSlot}{" "}
                menu
                {form.dayOfWeek !== "daily"
                  ? ` on ${form.dayOfWeek}s`
                  : " every day"}
                .
              </div>
...
function CloudKitchenMenu({
  ...
  const dayItems =
    view === "weekly"
      ? slotItems.filter(
          (i) => i.dayOfWeek === selDay || i.dayOfWeek === "daily",
        )
      : slotItems;
  ...
  return (
    <div className="space-y-4">
            ...
            >
              ...
              <span className="text-[10px] opacity-60">
                (
                {
                  menu.filter(
                    (i) =>
                      (i.mealSlot || "lunch") === s.val ||
                      (i.mealSlot || "lunch") === "both",
                  ).length
                }
                )
              </span>
          ...
          {DAYS.map((day) => (
            ...
            >
              ...
              <span className="ml-1 opacity-60 text-[9px]">
                ...
                {
                  slotItems.filter(
                    (i) => i.dayOfWeek === day || i.dayOfWeek === "daily",
                  ).length
                }
                )
              </span>
      ...
      >
        ...
        <div>
          <p
            className={`font-bold ${slot === "lunch" ? "text-orange-700" : "text-blue-700"}`}
          >
            ...
            >
              <div className="relative h-36 bg-gray-100 overflow-hidden">
                ...
                <div className="absolute top-2 right-2">
                  <span
                    className={`text-[10px] font-black px-2 py-1 rounded-full ${
                      (item.mealSlot || "lunch") === "lunch"
                        ? "bg-orange-500 text-white"
                        : (item.mealSlot || "lunch") === "dinner"
                          ? "bg-blue-500 text-white"
                ...
                <div className="absolute top-2 left-2">
                  <span
                    className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center bg-white ${
                      item.isveg !== false
                        ? "border-green-600"
                        : "border-red-600"
                    }`}
                  >
                    <span
                      className={`w-2.5 h-2.5 rounded-full ${item.isveg !== false ? "bg-green-600" : "bg-red-600"}`}
                    />
                  </span>
                </div>
              </div>
              ...
              <div className="p-3">
                ...
                <div className="flex items-center gap-2 mt-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 py-1.5 text-xs font-bold rounded-lg border border-gray-200 hover:bg-gray-50 transition"
                  >
                    {item.isAvailable ? (
                  ...
                  <button
                    onClick={() => onEdit(item)}
                    className="p-2 rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-200 transition text-blue-500"
                  >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-gray-50">
                {DAYS.map((day) => {
                  const dayMenu = slotItems.filter(
                    (i) => i.dayOfWeek === day || i.dayOfWeek === "daily",
                  );
                  return (
                    <tr
...
function RestaurantMenu({
  ...
  const filtered = menu.filter((item) => {
    const matchCat =
      filterCat === "all" ||
      (item.category?._id || item.category) === filterCat;
    const matchSearch =
  ...
  const grouped = categories.reduce((acc, cat) => {
    const items = filtered.filter(
      (i) => (i.category?._id || i.category) === cat._id,
    );
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {categories.map((c) => (
            ...
            >
              {c.name} (
              {
                menu.filter((i) => (i.category?._id || i.category) === c._id)
                  .length
              }
              )
            </button>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <button
            onClick={() => onAdd(null)}
            className="mt-4 px-5 py-2 bg-orange-500 text-white font-bold text-sm rounded-xl"
          >
            Add First Item
          </button>
        </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1">
                      <div className="flex items-center gap-1.5">
                        <span
                          className={`w-3.5 h-3.5 border-2 rounded-sm flex-shrink-0 flex items-center justify-center ${
                            item.isveg !== false
                              ? "border-green-600"
                              : "border-red-600"
                          }`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${item.isveg !== false ? "bg-green-600" : "bg-red-600"}`}
                          />
                        </span>
                      ...
                      <span className="text-sm font-black text-orange-500 flex-shrink-0">
                        ₹{item.price}
                      </span>
                    </div>
                    {item.description && (
                    ...
                    <div className="flex gap-2 mt-2">
                      <button
                        onClick={() => onToggle(item)}
                        className="flex items-center gap-1 text-[10px] font-bold text-gray-500 hover:text-gray-700 transition"
                      >
                        {item.isAvailable ? (
                          <FaToggleOn className="text-green-500 text-base" />
                        ) : (
                      ...
                      <button
                        onClick={() => onEdit(item)}
                        className="ml-auto p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg transition"
                      >
                        <FaEdit className="text-xs" />
                      </button>
                      <button
...
export default function Menu({ isCloudKitchen }) {
  ...
  const handleAdd = (defaultSlot) =>
    setModal({ item: null, defaultSlot: defaultSlot || "lunch" });
  const handleEdit = (item) =>
    setModal({ item, defaultSlot: item.mealSlot || "lunch" });

  const handleToggle = async (item) => {
    try {
      await API.patch(`/vendor/menu/${item._id}/availability`, {
        ...
        {isCloudKitchen && (
          <div className="flex gap-3">
            {[
              ...
              <div
                key={label}
                className={`${bg} rounded-xl px-4 py-2 text-center`}
              >
