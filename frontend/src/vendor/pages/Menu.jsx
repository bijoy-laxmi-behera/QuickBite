...
export default function Menu({ isCloudKitchen }) {
  ...
  const fetchAll = async () => {
    try {
      ...
      const flat = Array.isArray(menuData)
        ? menuData
        : Object.values(menuData).flat();
      setMenu(flat);

      // If vendor has no own categories, load platform-wide ones
      const vendorCats = c.data.data || [];
      if (vendorCats.length > 0) {
        setCategories(vendorCats);
      } else {
        const platform = await API.get("/customer/categories");
        setCategories(platform.data.categories || platform.data.data || []);
      }
    } catch {}
    setLoading(false);
  };
