...
function ItemModal({ item, categories, isCloudKitchen, defaultSlot, onClose, onSave }) {
  const [form, setForm] = useState(
    item ? {
      ...
      mealSlot: defaultSlot || "lunch", dayOfWeek: "daily", image: null,
    }
  );
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[92vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          ...
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-xl hover:bg-gray-100 text-gray-500">
            <FaTimes />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          {/* Price + Category — Restaurant only */}
          {!isCloudKitchen && (
            <div className="grid grid-cols-2 gap-3">
              <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              { v: false, l: "Non-Veg",    icon: <FaDrumstickBite />, cls: "border-red-400 bg-red-50 text-red-700"       },
            ].map((opt) => (
          ...
          {/* Available toggle — Restaurant only */}
          {!isCloudKitchen && (
...
// ── Cloud Kitchen Menu ────────────────────────────────────────────────────────
function CloudKitchenMenu({ menu, categories, onAdd, onEdit, onDelete, onToggle }) {
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-xl px-4 py-3 text-sm flex items-start gap-3 ${
        ...
        <div>
          <p className={`font-bold ${slot === "lunch" ? "text-orange-700" : "text-blue-700"}`}>
            {slot === "lunch" ? "Lunch Menu" : "Dinner Menu"}{view === "weekly" ? ` — ${selDay}` : " — Daily"}
          </p>
          <p className="text-xs text-gray-500 mt-0.5">
            {view === "daily"
              ? "Items marked 'daily' appear every day."
      ...
      <button onClick={() => onAdd(slot)}
        className={`flex items-center gap-2 px-4 py-2.5 text-white font-bold text-sm rounded-xl transition shadow-lg ${
          slot === "lunch" ? "bg-orange-500 hover:bg-orange-600 shadow-orange-200" : "bg-blue-500 hover:bg-blue-600 shadow-blue-200"
        }`}>
      ...
      {dayItems.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-400 font-medium mt-3">No {slot} items {view === "weekly" ? `for ${selDay}` : "yet"}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {dayItems.map((item) => (
            <div key={item._id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition ${item.isAvailable ? "border-gray-100" : "border-gray-200 opacity-60"}`}>
              <div className="relative h-36 bg-gray-100 overflow-hidden">
                ...
                <div className="absolute top-2 right-2">
                  <span className={`text-[10px] font-black px-2 py-1 rounded-full ${
                    ...
                    {(item.mealSlot||"lunch") === "lunch" ? "🌤️ Lunch" : (item.mealSlot||"lunch") === "dinner" ? "🌙 Dinner" : "🌤️🌙 Both"}
                  </span>
                </div>
                <div className="absolute top-2 left-2">
                  <span className={`w-5 h-5 border-2 rounded-sm flex items-center justify-center bg-white ${item.isveg !== false ? "border-green-600" : "border-red-600"}`}>
              ...
              <div className="p-3">
                ...
                <div className="flex gap-1.5 mt-1.5 flex-wrap">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${item.dayOfWeek === "daily" ? "bg-gray-100 text-gray-500" : "bg-purple-100 text-purple-600"}`}>
                    📅 {item.dayOfWeek === "daily" ? "Every Day" : item.dayOfWeek}
                  </span>
                </div>
                {item.description && <p className="text-xs text-gray-400 mt-1.5 line-clamp-1">{item.description}</p>}
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <button onClick={() => onEdit(item)} className="p-2 rounded-lg border border-gray-200 hover:bg-blue-50 hover:border-blue-200 transition text-blue-500"><FaEdit className="text-xs" /></button>
                  <button onClick={() => onDelete(item._id)} className="p-2 rounded-lg border border-gray-200 hover:bg-red-50 hover:border-red-200 transition text-red-500"><FaTrash className="text-xs" /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {view === "weekly" && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mt-4">
          <div className="px-5 py-3 border-b border-gray-50">
            <h3 className="font-black text-gray-800 text-sm">Weekly {slot === "lunch" ? "Lunch" : "Dinner"} Schedule Overview</h3>
          </div>
                ...
                {DAYS.map((day) => {
                  ...
                  return (
                    <tr key={day} className="hover:bg-gray-50 transition cursor-pointer" onClick={() => setSelDay(day)}>
                      ...
                      <td className="px-4 py-2.5 text-gray-500 max-w-xs truncate">{dayMenu.slice(0, 3).map((i) => i.name).join(", ") || "—"}</td>
                      <td className="px-4 py-2.5">
                        <span className={`font-black px-2 py-0.5 rounded-full ${dayMenu.length > 0 ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400"}`}>
...
function RestaurantMenu({ menu, categories, onAdd, onEdit, onDelete, onToggle }) {
  ...
  const uncategorized = filtered.filter((i) => {
    const catId = i.category?._id || i.category;
    return !catId || !categories.find((c) => c._id === catId);
  });
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <button onClick={() => onAdd(null)} className="mt-4 px-5 py-2 bg-orange-500 text-white font-bold text-sm rounded-xl">Add First Item</button>
        </div>
      ) : (
        Object.entries(grouped).map(([catName, items]) => (
          <div key={catName}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {items.map((item) => (
                <div key={item._id} className={`bg-white rounded-2xl border overflow-hidden shadow-sm hover:shadow-md transition flex gap-3 p-3 ${item.isAvailable ? "border-gray-100" : "border-gray-200 opacity-60"}`}>
                  <div className="relative w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
                    <img src={item.image || "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=200&q=80"} alt={item.name} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-1">
                      <div className="flex items-center gap-1.5">
                        <span className={`w-3.5 h-3.5 border-2 rounded-sm flex-shrink-0 flex items-center justify-center ${item.isveg !== false ? "border-green-600" : "border-red-600"}`}>
...
export default function Menu({ isCloudKitchen }) {
  ...
  const fetchAll = async () => {
    try {
      ...
      setCategories(c.data?.data || c.data?.categories || []);
    } catch (e) {
